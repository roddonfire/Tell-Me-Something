...
visits_db = '__HOME__/final/esp_instructions.db'
...
def request_handler(request):
        ...
        try:
            if request['is_json'] == True:
                ...
                if update == "T": #updating an existing row in databse
                    ...
                    request_num = int(json_data['request_id'])
    ...
    if request['method' ] == "GET":
        if 'esp_id' in request['args']: #want only incomplete tasks for a specific esp; request coming from ESP
            ...
            result = c.execute('''SELECT * FROM esp_instructions WHERE esp_id = ? AND completed = 0 ORDER BY request_time DESC;''', (esp,)).fetchone()
            conn.commit()
            conn.close()
            try:
                request_id = result[0]
                ...
                return ""
        elif 'show_all' in request['args']: # debugging only
            result = c.execute('''SELECT * FROM esp_instructions ''').fetchall()
            return json.dumps(result)
        elif 'delete_all' in request['args']: # debugging only, careful!
            c.execute('''DELETE FROM esp_instructions; ''')
            conn.commit()
            conn.close()
        else: #want all tasks for all esps, both complete and incomplete; request coming from website
